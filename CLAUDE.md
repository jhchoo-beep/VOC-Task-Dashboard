...
| `week_start` | 날짜(월요일) | 🔴 **이름과 달리 구간의 '끝'이다.** 주 버킷이 덮는 구간은 화요일~월요일 7일이고 그 마지막 월요일이 라벨이다 — `2026-07-27` = `07-21(화)~07-27(월)`. 정본은 수기 입력(매주 월요일 아침에 '지난 7일'을 넣어 왔다). 계산은 `lib/otaDetail.ts`의 `weekLabelOf()` 하나뿐이며, **'월요일로 내리는' 옛 동작으로 되돌리면 수기 15행과 6일 어긋난다**(2026-07-27 전환, 5·6·7월 파생 전량 재파생 완료) |
