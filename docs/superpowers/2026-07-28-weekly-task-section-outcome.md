...
- 「채택 대기」 과제를 다시 볼 화면이 없다. 채택하면 이월에서 빠지므로 자기 주로 되돌아가야만
  보인다
- ~~`/voc-analysis` 쿼리 D의 월 경계 규약 미확정~~ → **2026-07-28 해결(재헌 확정)**: 달로 거르지
  않는다. `escalated = true`를 전부 가져오고, 지난 달에 이미 반영한 건은 Step 2.5 게이트에서
  사용자가 걸러낸다. 달로 거르면 월말 주(07-28~08-03, 라벨 08-03) 채택 건이 7월 분석에서 빠지고
  9월 초에야 잡히는데, 채택하는 순간 주간 이월에서도 빠져 그 사이 어디에도 안 보이기 때문.
  스킬 v2.2.0에 반영
